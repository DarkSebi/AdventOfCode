...
void main() {
  ...
  for(int currentLineIndex = 0; currentLineIndex < lines.length; currentLineIndex++) {
    ...
    int sumValidPartNumbers = getSumValidPartsCurrentLine(prevLine, curLine, nextLine);
 ...
 */
int getSumValidPartsCurrentLine(String? prevLine, String curLine, String? nextLine) {
  ...
  for(BeginEndIndex beginEndIndex in beginEndIndexList) {
    ...
    String surroundingNumber = surroundingPrevLine + surroundingCurLine + surroundingNextLine;

    sumValidPartNumbers += computeValidPartNumbers(surroundingNumber, surroundingCurLine);
  }
...
List<BeginEndIndex> getBeginAndEndIndex(String curLine) {
  ...
  for(int currentCharIndex = 0; currentCharIndex < curLine.length; currentCharIndex++) {

    print(currentCharIndex);

    // search for index of begin and end of number
    if(curLine[currentCharIndex].contains(digitRegExp) && lookingForBeginIndex) {
      beginIndex = (currentCharIndex == 0) ? 0 : (currentCharIndex - 1);
      lookingForBeginIndex = false; // start search for end of number
    }
    if((!curLine[currentCharIndex].contains(digitRegExp) && !lookingForBeginIndex) || (currentCharIndex == 1)) {
    ...
    // has found number
    if(foundNumber) {
      BeginEndIndex beginEndIndexEntry = new BeginEndIndex(beginIndex!, endIndex!);
 ...
 */
int computeValidPartNumbers(String surroundingNumber, String surroundingCurLine) {
  ...
  for(int cur = 0; cur < surroundingNumber.length; cur++) {
    ...
    if(!surroundingNumber[cur].contains(validRegExp) && surroundingNumber[cur] != '.') {
      isValidPart = true;
      break;
    }
  }
